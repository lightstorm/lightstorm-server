...
public class ItemDefinition {
	...
	public static void init() throws IOException {
		...
		final RandomAccessFile raf = new RandomAccessFile(
				"data/itemDefinitions.bin", "r");
		try {
			final ByteBuffer buffer = raf.getChannel().map(MapMode.READ_ONLY,
					0, raf.length());
			...
			for (int i = 0; i < count; i++) {
				...
				final boolean noted = buffer.get() == 1 ? true : false;
				int parentId = buffer.getShort() & 0xFFFF;
				if (parentId == 65535) {
					parentId = -1;
				}
				final boolean noteable = buffer.get() == 1 ? true : false;
				int notedId = buffer.getShort() & 0xFFFF;
				if (notedId == 65535) {
					notedId = -1;
				}
				final boolean stackable = buffer.get() == 1 ? true : false;
				final boolean members = buffer.get() == 1 ? true : false;
				final boolean prices = buffer.get() == 1 ? true : false;
				int shop = -1;
				int highAlc = -1;
				int lowAlc = -1;
				if (prices) {
				...
				definitions[i] = new ItemDefinition(i, name, examine, noted,
						noteable, stackable, parentId, notedId, members, shop,
						highAlc, lowAlc);
			}
