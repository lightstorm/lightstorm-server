...
public class World {
	...
	private void loadConfiguration() throws IOException,
		...
		try {
			...
			final Map<String, Map<String, String>> complexMappings = p
					.getComplexMappings();
			if (complexMappings.containsKey("packetHandlers")) {
				final Map<Class<?>, Object> loadedHandlers = new HashMap<Class<?>, Object>();
				for (final Map.Entry<String, String> handler : complexMappings
						.get("packetHandlers").entrySet()) {
					final int id = Integer.parseInt(handler.getKey());
					final Class<?> handlerClass = Class.forName(handler
							.getValue());
