...
public class LoginServerConnector extends IoHandlerAdapter {
	...
	public void connect(final String password, final int node) {
		...
		if (!cf.isConnected() && (session == null || !session.isConnected())) {
			...
			buf.flip();
			session.write(new LoginPacket(LoginPacket.AUTH, buf));
		}
	}
	...
	public LoginResult checkLogin(PlayerDetails pd) {
		...
		synchronized (checkLoginResults) {
			...
			final int code = checkLoginResults.remove(NameUtils
					.formatNameForProtocol(pd.getName()));
			if (code == 2) {
	...
	public boolean loadPlayer(Player player) {
		...
		synchronized (playerLoadResults) {
			...
			final IoBuffer playerData = playerLoadResults.remove(NameUtils
					.formatNameForProtocol(player.getName()));
			if (playerData == null) {
				return false;
			} else {
