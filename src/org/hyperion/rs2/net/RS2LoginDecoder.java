	...
	protected boolean doDecode(IoSession session, IoBuffer in,
		...
		case STATE_UPDATE:
			if (in.remaining() >= 4) {
				...
				OnDemandPool.getOnDemandPool()
						.pushRequest(
								new OnDemandRequest(session, cacheId, fileId,
										priority));
				return true;
			} else {
				in.rewind();
		...
		case STATE_OPCODE:
			if (in.remaining() >= 1) {
				...
				case OPCODE_UPDATE:
					...
					session.write(new PacketBuilder().put(INITIAL_RESPONSE)
							.toPacket());
					return true;
				default:
		...
		case STATE_CRYPTED:
			...
			if (in.remaining() >= size) {
				 ...
				 */
				final int uid = in.getInt();

				/*
				 * We read and format the name and passwords.
				 */
				final String name = NameUtils.formatName(IoBufferUtils
						.getRS2String(in));
				...
				final PlayerDetails pd = new PlayerDetails(session, name, pass,
