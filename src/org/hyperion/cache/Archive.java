...
public class Archive {
	...
	public Archive(CacheFile cf) throws IOException {
		...
		bb.position(0);
		final int uncompressed = (bb.get() & 0xFF) << 16
				| (bb.get() & 0xFF) << 8 | bb.get() & 0xFF;
		final int compressed = (bb.get() & 0xFF) << 16 | (bb.get() & 0xFF) << 8
				| bb.get() & 0xFF;
		...
		for (int i = 0; i < dataSize; i++) {
			...
			final int uncompressedSize = (bb.get() & 0xFF) << 16
					| (bb.get() & 0xFF) << 8 | bb.get() & 0xFF;
			final int compressedSize = (bb.get() & 0xFF) << 16
					| (bb.get() & 0xFF) << 8 | bb.get() & 0xFF;
			final ArchiveFile nf = new ArchiveFile(nameHash, uncompressedSize,
					compressedSize, off);
	...
	private byte[] decompress(byte[] data) throws IOException {
		...
		try {
			final InputStream is = new BZip2CompressorInputStream(
